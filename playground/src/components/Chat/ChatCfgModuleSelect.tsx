...
export function RemoteModuleCfgSheet() {
    ...
    const metadata = React.useMemo(() => {
        ...
        if (selectedGraph) {
            for (const key of Object.keys(installedAndRegisteredModulesMap)) {
                ...
                const hasMatchingNode = selectedGraph.nodes.some((node) => node.name.includes(moduleTypeKey))
    ...
    return (
        <Sheet>
            <SheetTrigger
                className={cn(buttonVariants({ variant: 'outline', size: 'icon' }), 'bg-transparent')}
            >
                <BoxesIcon />
            </SheetTrigger>
            <SheetContent className="w-[400px] overflow-y-auto sm:w-[540px]">
                <SheetHeader>
                    ...
                    <SheetDescription>
                        You can adjust extension modules here. The values will be written to the
                        property.json file.
                    </SheetDescription>
                </SheetHeader>
                    ...
                    <GraphModuleCfgForm
                        ...
                        onUpdate={async (data, tools) => {
                            try {
                                ...
                                const geminiV2VNode = GraphEditor.findNodeByPredicate(
                                    selectedGraphCopy,
                                    (node) => node.addon === 'gemini_v2v_python',
                                )
                                if (geminiV2VNode) {
                                ...
                                const currentToolsInGraph = nodes
                                    .filter((node) =>
                                        installedAndRegisteredToolModules.map((module) => module.name).includes(node.addon),
                                    )
                                    .map((node) => node.addon)
                                                                                    ...
                                                                                    <DropdownMenuItem
                                                                                        ...
                                                                                        onClick={() => {
                                                                                            if (!selectedTools.includes(module.name)) {
                                                                                                setSelectedTools((prev) => [...prev, module.name])
                                                                                            }
                                                                                        }}
                                                                                    >
                                                                                        {module.name}
